...
class Mecanum(
    ...
    fun followPathCMD(path: Path, holdEnd: Boolean): Command {
        return InstantCommand({ follower.follow(path) })
            .interruptOn { follower.isBusy.not() }
            .whenFinished { if (holdEnd) { follower.holding() } }
    }
